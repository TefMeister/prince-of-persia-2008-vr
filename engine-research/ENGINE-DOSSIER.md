...
**✅ 2026-09-04 (`/pd`, no launch) — CONFIRMED IN PC DATA, not just a console report.** `DataPC.forge
→ Game Bootstrap` ships two debug-menu datablocks matching the exe's own class names:
`StartMenuDebug_m` (embedded string `P_MainMenuDebug`) and `DebugMenuHandler_m` (embedded string
`P_PauseMenuDebug`, sitting next to the normal `P_PauseMenu`). **Both are directly registered in
the game's own `InterfaceManager 'Interface Manager'` datablock**, the same top-level registration
pattern already confirmed for the CGST state registry — i.e. these are live, wired-in menu screens
on PC, not orphaned or stripped data. `[verified-numerically 2026-09-04]` **Still not established:
the link from these menu blocks to the `CGST_DebugMode` state (188) itself** — none of the four
handler blocks reference the CameraRule/GraphRuleBook chain directly; the wiring likely runs
through an `ActionBlock` (2,464 estate-wide) not yet searched. Evidence:
...
- **`GraphRuleBook "MENU RuleBook"` and its children are the menu open/close visual-FX graph, not
  ...
  Looks tempting given the "MENU" name, but it is a different subsystem entirely; none of its ids
  appear in the debug-menu handler blocks. Don't re-walk this chain looking for `CGST_DebugMode`.
